...
//    echo $first . " " . $last . " : ";
    if ($last == "" or $first == ""){
        echo 'no first or last name <br/><br/>';
    } else {
...
//    echo $query . "</br></br>";
        ...
        if ($row['GeneralNotes']) {
            $query = "INSERT INTO Referrals.RecordNote(RecordID, UserID, Note, UserGroup) VALUES ('" . $testVal[0] . "','UPLOAD', '" . str_replace("'", "\'", $row['GeneralNotes']) . "', 'MA')";
            $conReferrals->query($query);
            if ($conReferrals->error) {
                echo $conReferrals->error . "<br/>";
            }
        }


        if ($row['ProviderResponce']) {
            $query = "INSERT INTO Referrals.RecordNote(RecordID, UserID, Note, UserGroup) VALUES ('" . $testVal[0] . "','UPLOAD', '" . str_replace("'", "\'", $row['ProviderResponce']) . "', 'Provider')";
            $conReferrals->query($query);
            if ($conReferrals->error) {
                echo $conReferrals->error . "<br/>";
            }
